...
| $b_m$ | 테두리 폭 (용접선 또는 가스켓) | mm | | $=(D_p-D_e)/2$ 이면 생략 가능 |
| 테두리 형식 | 레이저 용접 / 가스켓 / 브레이징 | – | | 경계조건 결정에 필요 |
| $N_{port}$ | 포트 수 | – | | SPHX는 보통 2 |
| $d_{port}$ | 포트 구멍 지름 | mm | | |
| $e_{port}$ | 판 중심에서 포트 중심까지 거리 | mm | | 포트 간 거리 $=2e_{port}$ |
| 포트 주변 | 포트 주위 무주름(평탄) 영역 또는 분배 패턴 유무 | – | | 있으면 대략 지름 |
| $\beta$ | 쉐브론 각 (능선–유동축) | ° | | 기준(유동축/수평축) 명시 |
| 쉐브론 구성 | 단일 V(apex 1개) / 다중 W(apex 여러 개) | – | | 다중이면 apex 개수·간격 |
| apex 형상 | 꺾임부 처리 (날카로운 꺾임 / 라운드 / 평탄 띠) | – | | 국부 응력에 영향 |
...
| $b_{ch}$ | 채널 간극 (유로 높이) | mm | | 고온측/저온측 다르면 각각 |
| 접촉 | 인접 판 산–산 접촉 여부, 접촉점 용접/브레이징 여부 | – | | 확장 단계(스택 강성)용 |
| 유로 구성 | 1-pass / 2-pass, 플레이트 팩 체결 방식(쉘 내 자유 / 타이로드) | – | | 경계조건용 |
