...
| 파일 | 내용 |
|---|---|
| `report.md` | 계산 보고서: 경고, 전처리된 입력, 형상 파생량, 국부축 채택식과 모델 비교, 등가 두께·균질 단일층 상수, ±β 영역 강성과 조합 추정, 단위 합력 응답, 하중 케이스 판정, 압력차 케이스(§7b, `loads.dP_max` 입력 시, docs/12), 한계 |
...
| 시트 | 내용 |
|---|---|
| README | 사용법, 색 규칙(파랑+노랑 = 입력, 검정 = 수식, 녹색 = 시트 간 참조), 탭 색, 범위·한계 |
| Summary | 결과 요약 한 장: 입력 요약, 경고 7행, 채택식 A/D, 등가 두께·K_t, 하중 케이스 판정, 압력차 판정·Hertz, Check 오차 — 모두 다른 시트 참조 |
...
| Geometry | 전처리 4규칙, 원호+접선 해(모드 A: Newton 20회 반복표 — $F(\alpha)=R_s(1-\cos\alpha)+(c-R_s\sin\alpha)\tan\alpha-H$, $F'=(c-R_s\sin\alpha)/\cos^2\alpha$, 반복값을 $(0.001, \alpha_{hi})$로 제한; 모드 B: α 입력 → R 선형해), $l, I_1, I_2, \lambda, J_1, J_2, A_u, I_u$, 무차원, 경고 7행(두께·t/R·H/p·수렴·입력 누락·입력 범위·β 범위) |
...
| Pressure | C-4 압력차: 접촉 격자·Hertz, 반주기 60분할 곡선 프레임 표(Castigliano, free/fixed), 끝점 정확값, 능선 연속보 VAM, 8점 중첩·판정 (docs/12) |
| Check | 파이썬 `results.json` 참조값(하드코딩) 대비 상대오차 28항목 + 압력 8항목(허용 2e-3, 60분할 이산화) — 기본값 입력에서만 의미 |

**파이썬 모듈과의 범위 차이**: 단면은 원호+접선만(사인형·사다리꼴·반원은 파이썬), 응력은 산·골 두 위치만(플랭크 분포는 파이썬). 6개 단위 하중 모두 최대 응력이 산 또는 골에 생기므로(§5) 두께 사이징 용도에는 충분하다.

**검증**: 이 환경에는 LibreOffice Calc가 없어 `formulas` 라이브러리(pip)로 워크북 전체를 평가했다. 시나리오 3(모든 분기: 외면 기준 H, 판축 방향 피치, 수평 기준 β, $R_c\ne R_v$, $t_{min}$·CA, 판축/좌측 하중 케이스, 골 고압면·fixed 압력)에서 형상·강성·회전·응력·압력 18항목이 파이썬과 일치(형상·강성 1e-7, 응력 3e-7, 압력 1e-3). 시나리오 4·5(모드 B에서 R_c 비움, 모드 A에서 R_c 비움, ΔP·허용응력 비움)에서 오류 셀 없이 경고 문구만 표시. 기본값에서 Check 28항목 최대 상대오차 $1.5\times10^{-7}$(파이썬의 800점 표본 위치 차이), 시나리오 2(모드 B α 입력 + 국부 $N_x=10$, 허용 200 MPa)에서 $K_t=17.000$, 산 내측 $\sigma_s/(N_x/h)=17.000$, 최대 von Mises 252.75 MPa, 판정 NG를 재현. 수식 오류 셀 0. 엑셀에서는 열 때 전체 재계산된다(`fullCalcOnLoad`).

**표면 명칭**: 응력 출력의 두 면은 +z면(ζ=+h/2, 판의 산 쪽 표면)과 −z면으로 부른다. 산에서는 +z면이 볼록면, 골에서는 오목면이므로 '외측/내측'이라는 말은 쓰지 않는다(파이썬 보고서 `fibre` 항목도 +z/−z).

**정의 이름 규칙(생성기)**: 셀 주소와 같은 이름(A11, D66, I1, C1, Z1), R1C1 참조 표기와 같은 이름(`Rc` = RC, R1C1 형태, R/C 단독)과 대소문자만 다른 이름(H_, h_; R_b, R_B)은 엑셀에서 쓸 수 없어 `A_11`, `R_crest`, `H_mid`, `h_calc` 등으로 치환하고, 생성 수식의 셀 참조는 모두 절대형(`$D$22`)으로 써서 이름 치환과 충돌하지 않게 했다. 생성기는 이름 규칙(`check_name`)과 대소문자 충돌을 검사해 위반 시 실패한다. 2026-09-11 실제 엑셀에서 `Rc`가 무효 이름으로 삭제되어 Geometry·Stress·Pressure 수식이 함께 제거되는 복구 오류가 있었고, 이 조치로 해결했다.
